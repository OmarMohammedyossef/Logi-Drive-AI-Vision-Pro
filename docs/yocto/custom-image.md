...
1. Clone the Raspberry Pi meta layer from OpenEmbedded:

    ```bash
    git clone -b kirkistone git://git.yoctoproject.org/meta-raspberrypi
    ```

2. Source the OpenEmbedded build environment.
...
3. Add a custom layer:
    ...
    - Or add it manually in `bblayer.conf`.

4. Add the necessary layers and recipes (see `bblayer.conf` below).
